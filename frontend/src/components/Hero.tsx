...
// 推荐常驻快捷显示的经典名著列表
const DEFAULT_TOP_BOOKS = ['西游记', '三国演义', '封神演义', '红楼梦']
...
export function Hero() {
  ...
  const allCharacters: HeroDeckCharacter[] = useMemo(() => {
    if (!characters || characters.length === 0) {
      return Object.entries(CANONICAL_PRESETS).map(([name, preset], idx) => ({
        id: String(idx + 1),
        name,
        source: '古典典籍',
        specialty: preset.specialty || '典籍名士 · 绝世风骨',
        grade: 'S',
        rarity: 'legendary',
        image: `/images/classics/characters/xiyouji-sun-wukong.png`,
        quote: preset.quote || '万卷浮生，千古名士入卷来。',
        description: preset.description || '典籍名士，拥有通天彻地之能。',
        power: 95,
        stats: { attack: 90, defense: 85, speed: 88, intelligence: 90, special: 92 },
        skills: ['法相天地', '神通广大'],
  ...
  return (
    <div className="relative w-full h-[calc(100vh-1rem)] overflow-hidden font-geist text-white select-none bg-[#0a0807] flex flex-col justify-between p-4 sm:p-6 lg:p-7">
      {/* 1. 背景氛围图层 (暗调水墨 + 当前角色景深模糊) */}
      {deckList.map((char, index) => (
        <div
          key={char.id}
          className={`absolute inset-0 bg-cover bg-center transition-opacity duration-1000 ease-out filter blur-2xl scale-110 opacity-25 ${
            safeActive === index ? 'opacity-35' : 'opacity-0 pointer-events-none'
          }`}
            ...
            return (
              ...
              >
                ...
                <div className="relative w-full h-full rounded-2xl overflow-hidden bg-gradient-to-b from-[#1c1815] via-[#120f0d] to-[#0d0a09] border border-amber-500/35 flex flex-col justify-between p-3.5 shadow-2xl">
                  ...
                  {/* 卡牌核心立绘区域 (完整显示角色头部与身躯，采用 object-contain 配合暗底) */}
                  <div className="absolute inset-0 z-10 flex items-center justify-center p-4 pt-7 pb-18">
                    {/* 角色背后光晕 */}
                    <div
                      className="absolute w-40 h-40 rounded-full filter blur-xl opacity-40 transition-colors"
                      style={{ backgroundColor: char.accentColor }}
                    />
                    <img
                      src={char.image}
                      alt={char.name}
                      className="relative z-10 w-full h-full object-contain filter drop-shadow-[0_12px_24px_rgba(0,0,0,0.85)] transition-transform duration-500 group-hover:scale-105"
                      loading="eager"
                      onError={(e) => {
